...
surv_data_git <- surv_data %>% 
  ...
  dplyr::select(IndID, weeks_date, status, sex = Sex,  
                birth_date, birth_year, last_date, death_year, death_month, 
...
length(unique(wolf_repro_final$Faehe)) # [1] 201
length(unique(wolf_repro_final$Territorium)) # [1] 165
nrow(wolf_repro_final) # [1] 564
head(wolf_repro_final)
```

```{r save reproduction data}
## save data for later use 
 # write.csv(wolf_repro_final, paste0(procdata_wd, "/data_wolf_reproduction_envconv_20230912.csv"),
 #           row.names = FALSE)
```
...
repro_data_git <- wolf_repro_final %>% 
  ...
  mutate(MaleID = case_when(
         Male == "" ~ NaN,
         TRUE ~ MaleID)) %>% 
  dplyr::select(TerrID, sYear, FemID, MaleID, Repro, Npups, hs_8km, nterr_50km, area_buffer50km, densTerr_50km, 
                N_Years_fem, First_year_fem, Delta_bfem)

write.csv(repro_data_git, paste0(procdata_wd, "/data_wolf_reproduction_table.csv"),
                 row.names = FALSE)

length(unique(repro_data_git$TerrID)) # 165
```
